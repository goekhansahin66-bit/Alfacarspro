...
function AlphaPanel({open,close,stock,termine,data,warns}) {
  ...
  const speak = (text) => {
    ...
    u.lang = "de-DE"; u.rate = 1.1; u.pitch = 1.0
  ...
  const startListening = () => {
    ...
    rec.onresult = (e)=>{
      ...
      setInp(text)
      if(e.results[0].isFinal) { setListening(false); send(text) }
    }
  ...
  const send = async (text) => {
    ...
    try {
      ...
      if(reply){hist.current.push({role:"assistant",content:reply});setMsgs(p=>[...p,{r:"a",t:reply,cl:true}]);speak(reply)}
    ...
    <div className={`alpha ${open?"open":""}`}>
      <div className="alpha-head">
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{width:34,height:34,borderRadius:"50%",background:"linear-gradient(135deg,var(--ac),#FF3D00)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,fontWeight:900,color:"white"}}>α</div>
          <div><div style={{fontWeight:700}}>Alpha</div><div style={{fontSize:10,color:speaking?"var(--ac)":"var(--gn)"}}>{speaking?"🔊 Spricht...":"● Live"}</div></div>
        </div>
        <div style={{display:"flex",gap:4}}>
          <button className={`btn btn-s ${voiceOn?"btn-p":""}`} onClick={()=>{setVoiceOn(!voiceOn);if(voiceOn)stopSpeaking()}} title="Stimme an/aus">{voiceOn?"🔊":"🔇"}</button>
          <button className="btn btn-s" onClick={close}><I n="x" s={14}/></button>
        </div>
      </div>
      <div className="alpha-msgs">
        {msgs.map((m,i)=><div key={i} className={`alpha-msg ${m.r==="u"?"u":"a"}`} style={m.cl?{borderLeft:"3px solid var(--gn)"}:{}}>
          {m.r==="a"&&<div style={{fontSize:9,fontWeight:700,color:m.cl?"var(--gn)":"var(--ac)",marginBottom:3}}>{m.cl?"ALPHA·CLAUDE":"ALPHA"}</div>}
          {m.t}
          {m.r==="a"&&m.cl&&<button onClick={()=>speak(m.t)} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,color:"var(--t3)",marginTop:4,display:"block"}}>🔊 Nochmal</button>}
        </div>)}
        {busy&&<div className="alpha-msg a"><span className="pulse">🧠...</span></div>}
        <div ref={end}/>
      </div>
      ...
      <div className="alpha-bar">
        <input value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder={listening?"Höre zu...":"Frag Alpha..."} style={{fontSize:15}}/>
        <button className={`btn btn-s ${listening?"btn-r":"btn-g"}`} onClick={listening?stopListening:startListening} style={{padding:"8px 12px",fontSize:18}}>
          {listening?"⏹":"🎤"}
        </button>
        {speaking&&<button className="btn btn-s btn-r" onClick={stopSpeaking} style={{padding:"8px 12px"}}>🔇</button>}
        <button className="btn btn-s btn-p" onClick={()=>send()}>→</button>
      </div>
    </div>
  )
}
